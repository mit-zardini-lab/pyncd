...
**The `R = Bool` target wrinkle.** §7.5 turns on the value-semiring parameter `R`: `R = ℝ` gives (`×`, then `Σ`) — the tensor/linear reading, realised over `Mat ℝ = FGModuleCat ℝ`; `R = Bool` gives (`∧`, then `∃`) — the predicate/relational reading. The *class* `TargetActegory _ _ Bool` typechecks (`Bool` is a `CommSemiring`), but the *default realization* does not: `Mat R = FGModuleCat R` requires `[CommRing R]` and `Bool` is only a `CommSemiring` (no additive inverses), so `Mat Bool` fails to elaborate. The predicate target therefore needs a separate `TargetActegory _ V Bool` over a relations / `Bool`-semimodule value category `V` (`∧`-then-`∃` Boolean matrix multiply) — recorded as a deferred formalization obligation.
