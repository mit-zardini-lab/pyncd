...
Equations with a trailing `.` — causal masked softmax in (4), RMSnorm in (7) and (11) — are expanded into explicit intermediate bindings. Equation (4) requires an intermediate `Score` relation because contracting `k` and then normalizing over `x'` are two separate aggregation steps. The causal filter (`xp <= x`) is applied as a `where` predicate so that future positions never enter the softmax denominator.
...
The score contracts over `k`; the `xp <= x` filter restricts the softmax denominator to causal positions only. An intermediate `Score` relation separates the contraction from the normalization.
...
# xp <= x filter applied before softmax: future positions never enter denominator.
vs = Float.ref()
