...
*Experiment-layer lifts (same structure, different layer).* Some constructs are data-independent lifts with an averaging/selecting output weave but tile a whole **training run** rather than a forward morphism, and aggregate an **estimator** rather than producing a deployed computation — so they live one layer up, beside model parallelism (execution layer), not in the forward-pass table. **k-fold cross-validation**: degree = the fold axis; the fold partition is a *fixed* split of the data indices (data-independent → a weave, not a `Route`); each sub-computation is "train on K−1 folds, evaluate on the held-out fold"; output weave = average the K metrics (mean ± variance). The same pattern covers **hyperparameter / grid / random search** (output weave = `argmax`/`max` over the metric — select-best), **nested CV** (two stacked experiment-level lifts), and **learning-curve / seed-variance studies** (tile over seed or train-set size). They differ only in the output weave, exactly as the inference-layer family does — but because the tiled thing is an optimization (an `argmin`, not a `Broadcasted`) and the output is a generalization estimate, they are *experiment-level* instances of the lift, not forward-pass architecture.
...
**Practical payoff — what "for free" does and doesn't mean.** "For free" means *no MoE-specific code is written*, not *works today*: it is conditional on the one-time refactor that makes the weave machinery parametric in `D` (roadmap §8 item 4.4) — until then `Weave`/`Broadcasted`/`bc_signature()` are hard-coded for `D = St`. With that refactor the *marginal* cost of dense MoE is ~zero, because it is a weave rather than a new construction rule:
...
- **Autoalignment (real).** An MoE layer composes into a `Composed`/TL chain via the same `@`/`Context` pushout as any `Broadcasted` (§2.1); interface discovery at `D = Br` must generalize as part of the 4.4 refactor.
- **Fusion (partial, and the pass is itself roadmap 2.2).** The experts' `Linear ; ReLU ; Linear` and a trailing residual-add fuse under the §4.1 conditions, but the **gate softmax is a fusion barrier** like any normalization (§4.1) — the whole layer does *not* collapse to one kernel.

The deeper benefit is comparative. A weave inherits *every* pass — not just these three but the equivariance audit (§5.3), shape inference (`Π_φ`, §3.2), dead-code elimination, serialization, and the `BrGraph` analyses — with no per-pass work. A *generator* pays bespoke cost in each pass, which is exactly what `Scan` does today. So §6.4's real practical content is a **design signal**: data-independent routing (dense MoE) is free; data-dependent routing (sparse MoE → a `Route` generator) is expensive in *every* pass. Prefer the former when an architecture should ride the existing infrastructure rather than extend it.
