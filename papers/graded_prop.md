...
Notation: `⊗` is monoidal product, `;` is diagrammatic-order composition (`f ; g` = first `f` then `g`), `I` is the monoidal unit, `Dᵒᵖ` the opposite category. We write `[X, P]` and `[f, P]` for the lift (defined below) to match [theory.md](theory.md), alongside the formal functor name `act`.
...
| --- | --- | --- |
| Operations are shape-typed wires, freely juxtaposed and permuted | [theory.md](theory.md#the-array-broadcasted-category-br) (Br as colored PROP) | the colored-PROP base ([§2](#2-preliminaries-colored-props)) |
| Each wire decomposes into index sub-wires; broadcasting lifts a base op over a loop | [theory.md](theory.md#weaves) (weaves, lifts) | the **lift action** `⊛` and **shape map** `sh` ([§3](#3-the-central-definition-d-graded-colored-prop)) |
| Structure (connectivity) separates from data (sizes, weights) | [acset.md](acset.md#the-grothendieck-construction) (Grothendieck) | `C ≅ ∫Dat` ([§5](#5-the-structuredata-split-as-a-grothendieck-construction)) |
| Composition unifies shared boundary | [theory.md](theory.md#autoalignment) (autoalignment), [acset.md](acset.md) (pushout) | composition-as-pushout ([§6](#6-composition-as-pushout)) |
| Compiling to a concrete target is a structure-preserving, parameter-carrying functor | [prop_ideas.md](prop_ideas.md) (algebra, Para) | **algebras** and the Para refinement ([§7](#7-algebras-construct-and-the-para-refinement)) |

The unifying observation ([future_ideas.md §2.4](future_ideas.md#24-weaves-are-not-a-br-thing--they-are-the-cartesian-lift-datum-of-a-graded-prop)) is that the weave is not a Br-specific gadget but the **cartesian-lift datum of a grading fibration `C → D`**. Making that precise is the content of [§3](#3-the-central-definition-d-graded-colored-prop).
...
The monoidal structure is unavoidable: placing two operations side by side *is* tensor product (list concatenation). The *symmetric* structure is what distinguishes a PROP from an operad: an operad allows permuting inputs but fixes output order; a PROP allows free permutation of both inputs and outputs. The same operation `f : X ⊗ Y → Z` can be composed against wires in any order, so both input and output lists must be freely permutable — that is the symmetry `σ`. Without it, two operations differing only in wire order would be genuinely distinct morphisms with no canonical bridge: axis permutation becomes a real tracked generator rather than a structural no-op, `construct()` cannot identify permutation-equivalent programs, and equivariance (Proposition 8.4) loses its subject — the symmetry monad has no symmetric structure on `C` to act on.
...
- **(Sh-⊗)** `sh*` is a monoid homomorphism (already imposed in [§3.1](#31-data)).
...
**Evaluation transformations.** For a point `p : I_D → P` in `D`, define `ev_p := act(−, p) : (− ⊛ P) ⇒ (− ⊛ I_D) ≅ Id_C`, the *slice at `p`*. Because `act` is a functor (Act-functor), each `ev_p` is automatically a **natural transformation**; its naturality square at `f : X → Y` is exactly [theory.md](theory.md#batch-lift-f-p-def-11)'s batch-lift law

> **(Eq. 3)** `[f, P] ; (Y ⊛ p) = (X ⊛ p) ; f`.

So Eq. 3 is *built in* by functoriality of `act` — it is not a separate axiom. The genuine content lives in **(Elem-C)**: elements (hence the `ev_p`) jointly separate morphisms, which is what pins down the weave ([§3.3](#33-weaves-as-cartesian-lift-data)).

**Which elementality slices use — `St`, not `Br`.** Two distinct uses of elementality must be kept apart, because they live on different categories. *Slice extraction* indexes by **points of `D = St`**: the `ev_p`/`(− ⊛ p)` slice above is taken at a point `p` of the degree `P ∈ Ob D`, and a general slice — the Pythonic `x[i, :, j]` of [weavesWiresMorphisms.pdf](weavesWiresMorphisms.pdf) Def. 10/Fig. 3 — is a `Br` reindexing `[a, η]` whose `η` is *built from* `St` elements (the coordinates `i`, `j` are points `I_D → A` of `St` axes) tensored with identities on the kept axes. `St` is *complete elemental* (its `El(A)` is the full set of index tuples), so it is `St` elementality that the executable target actually evaluates to pull out a slice. **(Elem-C)** — elementality of `C = Br` — is the *separate* statement that `Br`'s elements, which are *value-families* `(x_i)_{i ∈ El(A)}` ([weavesWiresMorphisms.pdf](weavesWiresMorphisms.pdf) Def. 9), separate `Br` *morphisms* (operations). That separation is what makes the weave a unique datum ([§3.3](#33-weaves-as-cartesian-lift-data) / Prop 8.2) — a property used to *reason about* operations, not a mechanism for extracting any individual slice. In short: slice extraction rests on `St` elementality (coordinates); `Br` elementality buys weave uniqueness.
...
The permutation relating the canonical order (targets first, tilings second) to the wire's actual sub-color order is the `Ω_w` of [theory.md](theory.md#weaves), recovered from the symmetry `σ`.
...
**Definition 4.1 (Generated graded PROP).** Fix a set `G` of *base generators* — degree-trivial morphisms with declared domain/codomain colors (for Br: `Linear`, `Einops`, `SoftMax`, `Elementwise`, `Embedding`, …). The *free `D`-graded colored PROP on `G`* is the closure of `G` under: PROP composition `;`, monoidal product `⊗`, symmetry `σ`, and the lift action `⊛` / reindexing — modulo only the axioms of [§3.2](#32-axioms). This is `C♯`, the **structural skeleton**: it carries connectivity and shape *symbols* but no concrete `D`-colors (sizes).
...
recovers the fully-sized graded PROP. An **acset instance** ([acset.md](acset.md)'s `SBrInstance`) is a finite presentation of a single `∫Dat`-morphism: its `C♯`-part is the connectivity, its `Dat`-part the `axis_sizes`/coefficients/datatypes. Serialization (CSV) writes the two parts to separate tables.
...
**What the framing does and does not buy.** It *certifies* the gluing — associativity, precise error semantics ("no cocone" = alignment failure; "inconsistent attributes" = size mismatch), and the hook into data migration — and it *is* the union-find, computed in near-linear time. It does **not** choose the interface (Stage 1). So [§6](#6-composition-as-pushout) is a **correctness/specification lens, not a composition algorithm**: pyncd already computes the pushout efficiently as union-find and would never invoke a generic colimit solver. The pushout explains and certifies what `Context` does; it does not replace it.
...
The source `C` carries a D-action `act` (Definition 3.1.2): lifting a morphism `f : X → Y` by a grading `P` gives `[f, P] : X ⊛ P → Y ⊛ P`. For `F : C → V` to respect this action — so that "evaluate then lift" equals "lift then evaluate" — `V` must carry a matching D-action `⊛_V`. Without it, the D-equivariance `F(X ⊛ P) ≅ F(X) ⊛_V P` is not stateable. Furthermore, the coherence laws of `⊛_V` must mirror those of `act` exactly, so that the equivariance isomorphisms commute with `υ`, `α`, and `δ` on both sides. A *target `D`-actegory* is precisely this matching structure on `V`.

**Why V must be symmetric monoidal.** The source `C` is a symmetric monoidal category — that is what it means to be a colored PROP. For `F : C → V` to be a *morphism of symmetric monoidal categories* (a PROP morphism), `V` must carry a matching symmetric monoidal structure: without a braiding on `V`, the symmetric component of `F` is not stateable, and Definition 7.2 degrades to a plain monoidal functor, not a PROP morphism. This is not cosmetic: the equivariance theorems (Proposition 8.4) require `F` to intertwine the symmetry monad on both sides; the Para 2-cell characterization of pass correctness and weight tying (Definition 7.3) requires the coherent braiding on `Para(V)`. The SMC requirement on `V` is inherited necessity — `C` being a colored PROP forces it.

**Definition 7.1 (Target `D`-actegory).** A *target* is a triple `(V, ⊛_V, R)` where `V` is a symmetric monoidal category, `R` is a commutative semiring called the *value semiring*, and `⊛_V : V × Dᵒᵖ ⥤ V` is a functor equipping `V` with a right `D`-actegory structure. The monoidal structure of `V` uses `R`: the tensor product `⊗_V` is the `R`-semimodule tensor product, so contraction (summing over contracted indices) uses `R`'s addition and element-wise combination uses `R`'s multiplication. The actegory structure consists of the functor `⊛_V` together with coherence isos `υ_V : (− ⊛_V I_D) ≅ Id_V` (unit) and `α_V : ((− ⊛_V P) ⊛_V Q) ≅ (− ⊛_V (Q ⊗ P))` (composition) — satisfying the same triangle and pentagon laws as [§3](#3-the-central-definition-d-graded-colored-prop) — **and distributivity isos** `δ_V : (X ⊗_V Y) ⊛_V P ≅ (X ⊛_V P) ⊗_V (Y ⊛_V P)` and `δ⁰_V : (I_V ⊛_V P) ≅ I_V` (the action is strong monoidal over `⊗_V`), natural and obeying the interchange/coherence laws of [§3.2](#32-axioms), exactly as the source `δ`/`δ⁰` of Definition 3.1(3). (These `δ_V`/`δ⁰_V` are what an algebra's `D`-equivariance commutes with in Definition 7.2.) The default is `R = ℝ` with the standard `(×, +)` semiring — multiply along shared indices, sum over contracted ones — which is the standard einsum semantics; a non-default `R` (e.g. `(ℝ, max, +)` tropical, `(Bool, ∧, ∨)` Boolean — multiplication then addition) changes contraction arithmetic, and — when `R` is a semiring that is not a ring (no additive inverses, as for tropical and Boolean) — changes the carrier `V` itself to a category of (f.g.) `R`-*semimodules* (relations / join-semilattices for the Boolean case), **not** vector spaces, while leaving the actegory coherence *laws* unchanged. So `V` depends on `R`; `V =` finite-dimensional vector spaces is specific to the default ring `R = ℝ`. For pyncd, `V` = finite-dimensional real vector spaces / PyTorch tensors with `R = ℝ` (the default), and `X ⊛_V P` appends dimensions of the sizes named by `P`.

**Definition 7.2 (Algebra).** An *algebra* is a functor `F : C → V` that is both a morphism of symmetric monoidal categories and a morphism of D-actegories — the two structures must be intertwined. Concretely, `F` is a **strong symmetric monoidal functor** `F : C → V` that is **`D`-equivariant**: natural isomorphisms `F(X ⊛ P) ≅ F(X) ⊛_V P` commuting with `υ, α, δ` and preserving `ev_p`. This is `construct()`. A **morphism of algebras** `F ⇒ F'` is a monoidal natural transformation.
...
**Definition 7.3 (Para refinement, 2-categorically).** Following CDL ([§11](#11-relation-to-categorical-deep-learning)), take `Para(V)` as a **2-category** (Gavranović et al.): 0-cells are objects, 1-cells are parametric maps `(P, f : P ⊗ A → B)`, and 2-cells `(P, f) ⇒ (P', f')` are **reparameterizations** — maps `r : P → P'` with `f = (r ⊗ A) ; f'`. Then `construct()` is a **2-functor** `Para(C) → Para(V)`, and three notions that otherwise live in separate vocabularies collapse into one — each is a 2-cell (reparameterization) in `Para(V)`:
...
- **Compiler-pass correctness** — a pass is correct iff it is a 2-cell: it transforms the forward map while fixing the parameter object up to reparameterization. This makes precise [future_ideas.md](future_ideas.md)'s slogan "a pass is correct iff it is a `Para` natural transformation."
...
**Proposition 8.1 (Lift functoriality / distribution).** `[f ; g, P] = [f, P] ; [g, P]` and `[f ⊗ g, P] = [f, P] ⊗ [g, P]`. *(Immediate from Act-functor + Dist; it is [theory.md](theory.md)'s "lifting distributes over composition and products.")*
...
**Proposition 8.3 (Grothendieck splitting).** `C ≅ ∫Dat` ([§5](#5-the-structuredata-split-as-a-grothendieck-construction)), with the structural skeleton `C♯` the base and the size-data the fiber. *(The acset structure/data split is literally the Grothendieck construction.)*

**Proposition 8.4 (Equivariance, via a symmetry monad).** Fix a *symmetry monad* `T` on `D` (or on its colors). An algebra `F` is **`T`-equivariant** iff it lifts to the Eilenberg–Moore category of `T` — i.e. `F` is a morphism of `T`-algebras. This is the CDL lens ([§11](#11-relation-to-categorical-deep-learning)): equivariance = morphism of algebras for the symmetry monad, recovering geometric deep learning. Instances:

- **Axis permutation (the batch-equivariance theorem).** When `T` is the free symmetric-group action, `F` commutes with permutations of tiling sub-colors — tiling axes are never seen by base operations, so equivariance in the `⊛`-variable forces it. This is [prop_ideas.md](prop_ideas.md)'s batch-equivariance theorem, now a corollary.
- **Node permutation / translation.** The free-group monad on a node axis gives GNN permutation-equivariance; a translation monad gives convolutional equivariance — domains the bare axis-permutation statement could not express. (GNN node-permutation is the *degree-permutation* case — [equivariance_unification.md §5.2](equivariance_unification.md#52-symmetry-acting-on-the-broadcasting-dropping-h4), where it is the PROP's own symmetry and recovers DeepSets.)
...
**Two routes to equivariance (resolved — see [equivariance_unification.md](equivariance_unification.md)).** This proposition encodes a symmetry as a *monad* `T` on `D`; [§9.3](#93-the-horizontal-axis-swapping-d) encodes one instead as a *grading* `D = BG` (the delooping of a group `G`), and [future_ideas.md Appendix A.2](future_ideas.md#a2-d--group-bg-base-repg-fibers) adds the fiber grading `D = Rep(G)`. The conjecture was that these coincide. They do, for group/monoid symmetries: all three compute one category, `Alg(C, V^{T_G})` — `C`-algebras valued in the `G`-objects of the target — because the action monad's Eilenberg–Moore category *is* the delooping presheaves, `V^{T_G} ≃ [BG, V] ≃ Rep(G)` (monoidally, when `T_G` is Hopf), and `Alg(C, −)` transports this equivalence. The scope is sharp: coincidence holds **iff** `T` is an action/Hopf monad (its presentation is `BG`) — the [recognition theorem](equivariance_unification.md#51-the-recognition-theorem--coincidence--action-monad); a symmetry monad carrying extra relations has `V^{T} ⊋ [BG, V]` and the routes genuinely diverge. Continuous `G` splits at compactness ([§5.3](equivariance_unification.md#53-continuous-g-compact-closable-vs-non-compact-open)): compact `G` is covered (Haar measure, Peter–Weyl), non-compact `G` is the open residue. Full statement and proofs (Phases 1–4) are in [equivariance_unification.md](equivariance_unification.md), which also unifies base ⊗ fiber via the action groupoid ([§4.1](equivariance_unification.md#41-base--fiber-the-action-groupoid-phase-3-completed)).

**Proposition 8.5 (Composition associativity).** Composition in `Inst(C♯)` is associative and unital. *(Pasting lemma for pushouts, [§6](#6-composition-as-pushout).)*
...
**Proposition 8.7 (`Scan` as a catamorphism — the positive home for species (i)).** The `Scan` excluded by Prop 8.6(i) has a positive home: it is the **catamorphism (fold)** accumulating the state trace (scanl) for the *parametric step endofunctor* `F` — carrying the per-step inputs as parameters — whose algebra structure map is the step morphism `step : F(H) → H`. Equivalently, `Scan` is a (lax) algebra for the free parametric monad on `F`, truncated at length `N` — the CDL account of recurrence ([§11](#11-relation-to-categorical-deep-learning)). Three consequences, none available from the bare generator of 8.6:

- **The prefix-restriction law is a corollary, not an axiom.** [theory.md](theory.md)'s prefix law (`Scan_N` restricted to the first `m` steps equals `Scan_m`) follows from the catamorphism universal property `cata(step) ∘ in = step ∘ F(cata(step))` and its uniqueness; it need not be posited separately.
...
**Proposition 8.8 (`Scan` batches).** Under the lift–fold distributivity law ([§3.4](#34-the-temporal-grading-and-making-scan-first-class)), `Scan` is closed under the actegory action by any degree `P` *orthogonal* to its temporal axis `L`: `act(Scan, P) ≅ Scan(act(step, P))`. *(So `Scan` participates in `ThreadedComposed` routing and the `vmap`/batch strategies despite not being a weave — 8.6's obstruction is only along `L`, never along orthogonal axes. This is what lets the compiler treat a batched recurrence as one fold run independently per batch coordinate.)*

---

## 9. Instantiation

Instantiation has two axes (cf. [future_ideas.md §6.4](future_ideas.md#64-stacking-levels-weaves-over-models-mixture-of-experts-ensembles)–[§6.5](future_ideas.md#65-swapping-the-index-d-as-a-dial-across-ml)): **vertical**, stacking the construction by letting `D` itself be a graded PROP ([§9.2](#92-the-speculative-third-level-d--br), `D = Br`), and **horizontal**, swapping `D` for a different index category ([§9.3](#93-the-horizontal-axis-swapping-d)). The base `D = St`, `C = Br` ([§9.1](#91-d--st-c--br)) sits at the origin of both.

### 9.1 `D = St`, `C = Br`

| Synthesis primitive ([§3](#3-the-central-definition-d-graded-colored-prop)–[§7](#7-algebras-construct-and-the-para-refinement)) | pyncd realization | Reference |
| --- | --- | --- |
...
| Target `D`-actegory `(V, R)` | PyTorch tensors with `R = ℝ` (default `(×, +)` semiring); `⊛_V` appends dimensions | torch_compile/ |
...
Every row is a definitional unfolding of [§3](#3-the-central-definition-d-graded-colored-prop)–[§7](#7-algebras-construct-and-the-para-refinement) with `D := St`, `C := Br`. The `Scan` extension is exactly the generator demanded by Proposition 8.6. Note that `St`'s affine reindexings are translations on `ℤⁿ`, so `D = St` is the **translation instance** of the group-graded family ([§9.3](#93-the-horizontal-axis-swapping-d)): ordinary convolution is already group convolution over the translation group.
...
| --- | --- | --- |
| `St` | affine stride | tensor programs, CNNs (translation-equivariant) — [§9.1](#91-d--st-c--br) |
| group `BG` / `Rep(G)` | group translation | equivariant & steerable nets; group convolution is a weave (the routes coincide — [equivariance_unification.md](equivariance_unification.md)) |
...
2. **`D`-graded colored PROP** (`Def 3.1`, [§3.2](#32-axioms)). A `structure` extending `ColoredPROP` with fields: `sh`, `act : (C × Dᵒᵖ) ⥤ C`, the iso fields `δ, δ⁰, υ, α`, and `Prop`-fields for (Sh-⊛), the action/dist coherences, (Elem-C), (Broadcast-gen). *Available:* functors, natural isos, monoidal-action scaffolding can be hand-rolled (Mathlib's `Action`/actegory support is partial). *To build:* the right-actegory coherence bundle — straightforward functor+nat-iso algebra.
...
5. **`C♯`, `P_arch`** ([§4](#4-generators-relations-and-the-structural-prop-c)). Free graded PROP on a generator quiver, then `CategoryTheory.Quotient` by `R`. *Available:* `FreeMonoidalCategory`, `CategoryTheory.Quotient`.

6. **Grothendieck splitting** ([§5](#5-the-structuredata-split-as-a-grothendieck-construction), `Prop 8.3`). Use `CategoryTheory.Grothendieck` on `Dat`. If `C` is *defined* as `∫Dat`, the iso is `Iso.refl`. *Available.*

7. **Composition as pushout** ([§6](#6-composition-as-pushout), `Prop 8.5`). `Inst(C♯)` with `CategoryTheory.Limits.pushout`; associativity from pasting. *Available* (pushout API, pasting lemmas).

8. **Algebras + Para** ([§7](#7-algebras-construct-and-the-para-refinement), `Prop 8.4`). `F : MonoidalFunctor C V` + a `D`-equivariance iso field; algebra morphisms `MonoidalNatTrans`. Para as a lightweight category of parameterized 1-cells. *Available:* `MonoidalFunctor`, `MonoidalNatTrans`; Para hand-rolled.
...
**Instantiation strategy.** Define `St` and `Br` as concrete `ColoredPROP`s, then provide the `GradedPROP St Br` instance by supplying `sh`, `act`, and discharging the `Prop`-fields. Because every [§3](#3-the-central-definition-d-graded-colored-prop)–[§7](#7-algebras-construct-and-the-para-refinement) construction is generic, all of [§8](#8-propositions-the-synthesis-organizes)'s propositions transfer to `Br` with no Br-specific proof, and a later `GradedPROP Br C_mod` instance (the MoE level) reuses the same propositions.
...
- *Functorial semantics.* CDL: a model is a (lax) algebra; here: a model is a **PROP-algebra** (a strong symmetric monoidal functor `F : C → V`, [§7](#7-algebras-construct-and-the-para-refinement)). Same Lawvere-style stance — an architecture is a theory, a trained network is an algebra of it.
- *Para for learning.* CDL works in the 2-category `Para`; parametric maps and weight tying are 2-categorical data. The functor `construct() : Para(Br) → Para(PyTorch)` and "training = a section of the Para fibration" ([§7](#7-algebras-construct-and-the-para-refinement)) use the same construction, and the weight-tying-as-diagonal of [future_ideas.md §5.2](future_ideas.md#52-weight-tying-comonoid-para-morphism-acset-parameter-group) is CDL's 2-categorical weight tying.

**What CDL does in greater generality (this framework specializes).**

- *Equivariance.* CDL models equivariance as **monad-algebra homomorphisms** and thereby "captures the entirety of Geometric Deep Learning" — far broader than the axis-permutation equivariance here, which is a corollary of algebra-morphism commutation (Prop 8.4; [future_ideas.md §5.3](future_ideas.md#53-equivariance-from-stridecategory-representation-theory)). CDL is the general theory; this is one computable special case. The bridge is made precise by the [recognition theorem](equivariance_unification.md#51-the-recognition-theorem--coincidence--action-monad): CDL's monad route equals the grading route here **iff** the monad is an action monad.
...
- *The index grading / actegory lift (St→Br, weaves).* CDL has no shape category acting by broadcasting — no degree, tiling/target weaves, or reindexings. This is the entire `D`-graded-colored-PROP content ([§3](#3-the-central-definition-d-graded-colored-prop)), driven by tensor *execution* (GPU tiling), a concern orthogonal to CDL's constraint-specification focus.
- *The acset structure/data split and compilation.* `C ≅ ∫Dat`, schema morphisms, shape inference as a Kan extension, and serialization ([§5](#5-the-structuredata-split-as-a-grothendieck-construction)) are the representational and compilation machinery CDL leaves open.
...
- **Weaves** are the cartesian-lift data of the grading ([§3.3](#33-weaves-as-cartesian-lift-data)); their well-definedness is a theorem (Prop 8.2), not an assumption.
- **Lifts** are the action `act`; their distribution laws are functoriality (Prop 8.1).
- **The structure/data split** is the Grothendieck construction `C ≅ ∫Dat` ([§5](#5-the-structuredata-split-as-a-grothendieck-construction), Prop 8.3).
- **Autoalignment** is pushout in the instance category ([§6](#6-composition-as-pushout), Prop 8.5).
- **`construct()`** is a `D`-equivariant strong symmetric monoidal algebra into a target actegory; the **Para refinement** carries the weights ([§7](#7-algebras-construct-and-the-para-refinement)); batch equivariance is a corollary (Prop 8.4).
...
`St → Br` is one instantiation (`D = St`, `C = Br`, [§9.1](#91-d--st-c--br)); the model level (`D = Br`, [§9.2](#92-the-speculative-third-level-d--br)) is another, awaiting only the Eq. 3 check for data-dependent routing. The definition is stated as data + named laws throughout, so a Lean development transcribes it as a `structure` and proves [§8](#8-propositions-the-synthesis-organizes) once, at the graded-PROP level, inheriting it at every instantiation.
...
- Baez & Courser, "Structured Cospans", *Theory and Applications of Categories* 35, 2020 (composition of open systems by pushout — [§6](#6-composition-as-pushout)).
- Capucci & Gavranović, "Actegories for the Working Amthematician", arXiv:2203.16351 (actegories / monoidal actions).
- Gavranović, Lessard, Dudzik, von Glehn, Araújo, Veličković, "Position: Categorical Deep Learning is an Algebraic Theory of All Architectures", arXiv:2402.15332 (the Para construction; monad-algebra equivariance; recursion as lax algebras for parametric monads). See [§11](#11-relation-to-categorical-deep-learning).
