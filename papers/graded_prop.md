...
**Definition 7.1 (Target `D`-actegory).** A *target* is a triple `(V, ⊛_V, R)` where `V` is a symmetric monoidal category, `R` is a commutative semiring called the *value semiring*, and `⊛_V : V × Dᵒᵖ ⥤ V` is a functor equipping `V` with a right `D`-actegory structure. The monoidal structure of `V` uses `R`: the tensor product `⊗_V` is the `R`-module tensor product, so contraction (summing over contracted indices) uses `R`'s addition and element-wise combination uses `R`'s multiplication. The actegory structure consists of the functor `⊛_V` together with coherence isos `υ_V : (− ⊛_V I_D) ≅ Id_V` (unit) and `α_V : ((− ⊛_V P) ⊛_V Q) ≅ (− ⊛_V (Q ⊗ P))` (composition), satisfying the same triangle and pentagon laws as [§3](#3-the-central-definition-d-graded-colored-prop). The default is `R = ℝ` with the standard `(×, +)` semiring — multiply along shared indices, sum over contracted ones — which is the standard einsum semantics; a non-default `R` (e.g. `(ℝ, max, +)` tropical, `(Bool, ∧, ∨)` Boolean) changes contraction arithmetic without altering the actegory structure. For pyncd, `V` = finite-dimensional real vector spaces / PyTorch tensors with `R = ℝ` (the default), and `X ⊛_V P` appends dimensions of the sizes named by `P`.
