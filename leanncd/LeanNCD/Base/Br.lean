...
The earlier free-list presentation (`nil`/`cons`) made the category laws trivial but could NOT
satisfy the morphism-level symmetric-monoidal laws: `swap a b ; swap b a` is a two-element list,
never definitionally `nil`, so `swap_swap` was unprovable (and likewise `tensorHom_comp`). Following
the theory (§2.3) there is also no single canonical *record* for a composite broadcast op, so `Br`
cannot be a concrete morphism type like `St`. The resolution is to present `Br` as the free strict
symmetric monoidal category: the symmetry (`braid`) and the tensor are first-class generators, and
morphisms are quotiented by the strict-SMC laws.
