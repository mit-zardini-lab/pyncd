...
A free-list of base operations cannot satisfy the morphism-level symmetric-monoidal laws: any
concrete list type has `swap a b ; swap b a` as a two-element sequence, never definitionally equal
to `nil`/`id`, so `swap_swap` is unprovable by `noConfusion`. Similarly there is no single
canonical record for a composite broadcast op (§2.3), so `Br` cannot be a concrete morphism type
like `St`. The only option is to work with raw syntax quotiented by equations — i.e. to present
`Br` as the free strict symmetric monoidal category: `braid` and `tensor` are first-class syntax
constructors, and morphisms are identified up to the strict-SMC laws. For example,
`.comp (.braid a b) (.braid b a)` and `.id (a ++ b)` are distinct `Hom` trees, but
`Rel.braid_involution` makes them equal in the quotient, so `swap_swap` holds by `Quot.sound`.

`Hom` is the raw syntax; `Rel` is the congruence generated by exactly the laws the `ColoredPROP`
fields need — the category laws, the bifunctor laws (`tensor_id`, interchange), and the braid
involution. (Braid *naturality* and the *hexagon* are NOT included here; they belong to the
`MonoidalCategory`/`SymmetricCategory` seam in `Seam/Adapter.lean` and are added there.) `BrMorph` is
the quotient; tensor (`tensorHom`) and braid (`swap`) descend, and `swap_swap`/`tensorHom_comp`/
`tensorHom_id` hold by `Quot.sound`. -/
...
/-- The hard content of `elemental`, isolated on raw syntax: **left-cancellation of a point**.
    ...
    OBLIGATION (`sorry`) — the strict-monoidal normal-form milestone. With the chosen `Rel`,
    `(Hom, comp, tensor, id)` is the free strict *monoidal* category on the generators (`BrBase` ops
    + `braid` symbols) plus `braid` involution. This lemma is TRUE (a generator participates in no
    `Rel` constructor, so a leading `gen (brPoint X)` cannot be rewritten away) but establishing it
    needs a `Rel`-respecting normal form on `Hom`. The `assoc`/`braid_involution` fragments are
    Spike-tractable (`scratchpad/Spike.lean`); the wall is **`interchange`-invariance** — the
    2-dimensional sliding of independent generators across monoidal layers, i.e. the strict-SMC
    word problem. A dedicated several-hundred-line normalization, deferred. -/
