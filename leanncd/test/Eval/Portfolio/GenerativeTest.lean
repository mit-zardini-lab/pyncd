...
run_cmd (assertEval "DF1 forward-process"
  (tlprog!{ Xt[i] := alpha[] · x0[i] + beta[] · eps[i] })
...
run_cmd (assertEval "DF2 ddpm-posterior"
  (tlprog!{ Xp[i] := c1[] · Xt[i] + c2[] · e[i] })
...
run_cmd (assertEval "DF3 denoising-loss"
  (tlprog!{
    r[i] := eps[i] + m1[] · ehat[i]
    sse[] := r[i] · r[i]
  })
...
run_cmd (assertEval "SS1 linear-recurrence"
  (tlprog!{
    axis l : ℕ = 3
    h[j, 0]    := h0[j]
...
-- C=[[1]] ⇒ y = h = [1,2,3].
run_cmd (assertEval "SS2 output-map"
  (tlprog!{ y[j, l] := C[j, k] · h[k, l] })
...
run_cmd (assertEval "SS3 diagonal-ssm"
  (tlprog!{
    axis l : ℕ = 3
    h[j, 0]    := h0[j]
...
--   ⇒ [[11,22],[33,44]].
run_cmd (assertEval "PE1 additive-pe"
  (tlprog!{ Out[p, d] := X[p, d] + PE[p, d] })
...
run_cmd (assertEval "CL1 similarity"
  (tlprog!{ S[i, j] := Z1[i, d] · Z2[j, d] })
...
run_cmd (assertEvalPred "CL2 infonce-softmax"
  (tlprog!{
    S[i, j] := Z1[i, d] · Z2[j, d]
    tensor P(i, j)
    P[i, j.] := softmax(S[i, j])
  ...
  (HashMap.ofList [("Z1", tl [2,2] [1,0,0,1]), ("Z2", tl [2,2] [1,0,0,1])])
  "P" rowsSumToOne "softmax rows sum to 1")
