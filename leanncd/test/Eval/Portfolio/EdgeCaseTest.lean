...
run_cmd (assertEval "EC1 lookback-1"
  (tlprog!{ Y[i] := X[i - 1] })
...
--   ⇒ [0,0,0,1,2,3].
run_cmd (assertEval "EC2 lookback-3"
  (tlprog!{ Y[i] := X[i - 3] })
...
run_cmd (assertEval "EC7 diagonal-write"
  (tlprog!{
    tensor D(i, j)
    D[i, i] := v[i]
  })
...
run_cmd (assertEval "EC10 high-rank"
  (tlprog!{ Y[i, j, k, l] := A[i, j, m] · B[m, k, l] })
...
run_cmd (assertEval "EC11 nary-product"
  (tlprog!{ Z[] := A[] · B[] · C[] · D[] })
...
run_cmd (assertEval "EC12 affine-reindex"
  (tlprog!{
    axis i : ℕ = 2, j : ℕ = 2
    Y[i] := X[2 * i + 3 * j - 1]
  })
...
--   Y=(A·B)+(C·D)=[[4,5],[7,9]].
run_cmd (assertEval "EC13 precedence"
  (tlprog!{ Y[i, j] := A[i, k] · B[k, j] + C[i, l] · D[l, j] })
  (HashMap.ofList [("A", tl [2,1] [1,2]), ("B", tl [1,2] [3,4]),
...
run_cmd (assertEval "EC14 rank5-contract"
  (tlprog!{ s[] := A[i, j, k, l, m] })
...
run_cmd (assertEval "EC15 equation-level-sum"
  (tlprog!{ Y[i] := u[] · a[i] + W[i, k] · v[k] })
