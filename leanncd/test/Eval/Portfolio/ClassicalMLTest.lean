...
run_cmd (assertEval "CM1 pairwise-sqdist"
  (tlprog!{
    sq[i] := X[i, d] · X[i, d]
...
run_cmd (assertEval "CM2 factorization-machine"
  (tlprog!{
...
run_cmd (assertEval "CM3 value-iteration"
  (tlprog!{
...
run_cmd (assertEval "CM4 power-iteration"
  (tlprog!{
    axis l : ℕ = 3
    p[j, 0] := p0[j]
    p[j, l +1] := p[i, l] · M[i, j]
...
run_cmd (assertEval "CM5 matrix-factorization"
  (tlprog!{ Rhat[u, i] := P[u, f] · Q[i, f] })
...
run_cmd (assertEval "CM6 vae-reparam"
  (tlprog!{ z[i] := mu[i] + sigma[i] · eps[i] })
