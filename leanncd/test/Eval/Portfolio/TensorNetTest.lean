...
run_cmd (assertEval "TN1 tensor-train"
  (tlprog!{ Y[a, d] := G1[a, b] · G2[b, c] · G3[c, d] })
...
run_cmd (assertEval "TN2 CP-reconstruction"
  (tlprog!{ T[i, j, k] := A[i, r] · B[j, r] · C[k, r] })
...
-- 1·1 + 2·0 + 3·0 + 4·1 = 5.
run_cmd (assertEval "TN3 frobenius"
  (tlprog!{ s[] := A[i, j] · B[i, j] })
...
run_cmd (assertEval "TN4 third-moment"
  (tlprog!{ M[i, j, k] := X[t, i] · X[t, j] · X[t, k] })
